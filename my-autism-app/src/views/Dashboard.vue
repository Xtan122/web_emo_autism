<template>
  <div class="flex h-screen bg-slate-50 font-quicksand overflow-hidden">
    ...
    <div class="flex-1 h-full overflow-y-auto relative no-scrollbar scroll-smooth">
      ...
      <div class="md:hidden sticky top-0 z-50 bg-white/90 backdrop-blur p-4 border-b border-slate-100 flex justify-between items-center">
        <span class="font-bold text-blue-600 text-xl">EmpathyKids</span>
        <div class="flex gap-3">
           <div class="text-orange-500 font-bold"><i class="fas fa-fire"></i> {{ store.currentStreak }}</div>
           <div class="text-yellow-500 font-bold"><i class="fas fa-star"></i> {{ store.stars }}</div>
        </div>
      </div>
        ...
        <div v-for="(level, index) in store.levels" :key="level.id" class="relative mb-20">
          ...
          <div class="flex justify-center mb-16 relative z-20">
             <div :class="['px-40 py-3 rounded-xl font-bold text-white shadow-lg text-base uppercase tracking-wider text-center min-w-[280px]', 
              level.locked ? 'bg-slate-400' : 'bg-green-500']">
          ...
          <div class="relative h-[850px] w-full">
            ...
            <svg class="absolute top-0 left-0 w-full h-full pointer-events-none z-0" viewBox="0 0 100 100" preserveAspectRatio="none">
              <path 
                d="M 50 5 C 50 15 20 15 20 25 C 20 35 80 35 80 45 C 80 55 20 55 20 65 C 20 75 50 75 50 85 L 50 98" 
                fill="none" 
                stroke="#cbd5e1" 
                stroke-width="2.5" 
            ...
            <div class="absolute left-1/2 -translate-x-1/2 top-[2%] z-20 group">
              <div v-if="getLessonState(level, 'flashcard') === 'current'" id="current-lesson-node" class="absolute -top-8 left-1/2 -translate-x-1/2 flex flex-col items-center z-30 w-32 animate-bounce">
                <div class="bg-white text-yellow-600 text-sm font-extrabold px-6 py-3 rounded-2xl shadow-xl border-2 border-yellow-100 uppercase tracking-wide">Bắt đầu</div>
              ...
              <button @click="goToLesson(level.id, 'flashcard')" :class="getNodeClass(level, 'flashcard')">
                ...
                <div v-if="level.lessons.flashcard" class="absolute -right-1 -bottom-1 bg-green-500 text-white w-7 h-7 rounded-full flex items-center justify-center text-sm border-4 border-white shadow-sm z-20"><i class="fas fa-check"></i></div>
              </button>
            </div>
            ...
            <div class="absolute left-[10%] top-[23%] z-20 group">
              <div v-if="getLessonState(level, 'matching') === 'current'" id="current-lesson-node" class="absolute -top-8 left-1/2 -translate-x-1/2 flex flex-col items-center z-30 w-32 animate-bounce">
                <div class="bg-white text-pink-600 text-sm font-extrabold px-6 py-3 rounded-2xl shadow-xl border-2 border-pink-100 uppercase tracking-wide">Bắt đầu</div>
                <div class="w-0 h-0 border-l-[8px] border-l-transparent border-r-[8px] border-r-transparent border-t-[10px] border-t-white -mt-[1px]"></div>
              </div>
              <button @click="goToLesson(level.id, 'matching')" :class="getNodeClass(level, 'matching')">
                <i class="fas fa-puzzle-piece"></i>
                <div v-if="level.lessons.matching" class="absolute -right-1 -bottom-1 bg-green-500 text-white w-7 h-7 rounded-full flex items-center justify-center text-sm border-4 border-white shadow-sm z-20"><i class="fas fa-check"></i></div>
              </button>
            </div>
            ...
            <div class="absolute right-[10%] top-[43%] z-20 group">
              ...
              <button @click="goToLesson(level.id, 'context')" :class="getNodeClass(level, 'context')">
                ...
                <div v-if="level.lessons.context" class="absolute -right-1 -bottom-1 bg-green-500 text-white w-7 h-7 rounded-full flex items-center justify-center text-sm border-4 border-white shadow-sm z-20"><i class="fas fa-check"></i></div>
              </button>
            </div>

            <div class="absolute left-[10%] top-[63%] z-20 group">
              <div v-if="getLessonState(level, 'emotion_training') === 'current'" id="current-lesson-node" class="absolute -top-8 left-1/2 -translate-x-1/2 flex flex-col items-center z-30 w-32 animate-bounce">
                <div class="bg-white text-orange-600 text-sm font-extrabold px-6 py-3 rounded-2xl shadow-xl border-2 border-orange-100 uppercase tracking-wide">Bắt đầu</div>
                <div class="w-0 h-0 border-l-[8px] border-l-transparent border-r-[8px] border-r-transparent border-t-[10px] border-t-white -mt-[1px]"></div>
              </div>
              <button @click="goToLesson(level.id, 'emotion_training')" :class="getNodeClass(level, 'emotion_training')">
                <i class="fas fa-solid fa-dumbbell"></i>
                <div v-if="level.lessons.emotion_training" class="absolute -right-1 -bottom-1 bg-green-500 text-white w-7 h-7 rounded-full flex items-center justify-center text-sm border-4 border-white shadow-sm z-20"><i class="fas fa-check"></i></div>
              </button>
            </div>
            ...
            <div class="absolute left-1/2 -translate-x-1/2 top-[83%] z-20 group">
              ...
              <button @click="goToLesson(level.id, 'ai')" :class="getNodeClass(level, 'ai')">
                ...
                <div v-if="level.lessons.ai" class="absolute -right-1 -bottom-1 bg-green-500 text-white w-7 h-7 rounded-full flex items-center justify-center text-sm border-4 border-white shadow-sm z-20"><i class="fas fa-check"></i></div>
              </button>
            </div>

            <div class="absolute left-1/2 -translate-x-1/2 top-[98%] z-30 group cursor-pointer transition-transform active:scale-90 active:translate-y-1" @click="handleOpenChest(level, $event)">
              <img v-if="!isLevelDone(level)" src="/images/chest/Color=Gold, Open=False.svg" class="w-20 h-22 grayscale opacity-60 drop-shadow-sm" />
              <div v-else-if="!level.chestClaimed" class="relative">
                 <div class="absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap bg-yellow-400 text-white text-xs font-bold px-6 py-3 rounded-xl animate-bounce border-2 border-white shadow-md z-40">Mở quà!</div>
                 <img src="/images/chest/Color=Gold, Open=False.svg" class="w-22 h-24 drop-shadow-2xl animate-pulse hover:scale-105 transition" />
              </div>
          ...
          <div class="flex flex-col items-center mt-20 relative z-0">
             <div v-if="index === store.levels.length - 1" class="h-20 w-0 border-l-4 border-dashed border-slate-300"></div>
    ...
    <div class="hidden lg:flex w-80 shrink-0 h-full border-l border-slate-200 bg-white flex-col p-6 gap-8 z-20">
      ...
      <div class="flex justify-between items-center gap-4">
        <div class="flex items-center gap-2 font-extrabold text-slate-400 hover:bg-slate-100 px-3 py-2 rounded-xl transition cursor-pointer">
           <i class="fas fa-fire text-orange-500 text-xl"></i> 
           <span class="text-orange-500">{{ store.currentStreak }}</span>
        </div>
        <div id="star-counter" class="flex items-center gap-2 font-extrabold text-slate-400 hover:bg-slate-100 px-3 py-2 rounded-xl transition cursor-pointer" :class="{ 'scale-125 text-yellow-500': starBump }">
           <i class="fas fa-star text-yellow-400 text-xl"></i>
           <span class="text-yellow-500">{{ store.stars }}</span>
        </div>
        <div class="flex items-center gap-2 font-extrabold text-slate-400 hover:bg-slate-100 px-3 py-2 rounded-xl transition cursor-pointer">
      ...
      <div class="border-2 border-slate-100 rounded-2xl p-4">
        <div class="flex justify-between items-center mb-4">
          ...
          <span class="text-xs font-bold text-blue-500 uppercase cursor-pointer hover:text-blue-600">Xem tất cả</span>
        </div>
        <div class="space-y-4">
          <div class="flex items-center gap-4">
            ...
            <div class="flex-1">
              ...
              <div class="w-full bg-slate-200 h-2.5 rounded-full mt-2 overflow-hidden"><div class="bg-orange-500 h-full w-1/2 rounded-full"></div></div>
            </div>
            <div class="text-xs font-bold text-orange-500">1/2</div>
          </div>
          <div class="flex items-center gap-4">
            <div class="text-2xl">🎁</div>
            <div class="flex-1">
              <p class="font-bold text-slate-700 text-sm">Mở 1 rương kho báu</p>
              <div class="w-full bg-slate-200 h-2.5 rounded-full mt-2 overflow-hidden"><div class="bg-green-500 h-full w-full rounded-full"></div></div>
            </div>
            <div class="text-green-600 font-bold text-xs"><i class="fas fa-check"></i></div>
          </div>
        </div>
      </div>

      <div class="border-2 border-slate-100 rounded-2xl p-4 relative overflow-hidden group cursor-pointer">
         <div class="absolute inset-0 bg-gradient-to-br from-purple-500 to-blue-500 opacity-10 group-hover:opacity-20 transition"></div>
         ...
         <button class="w-full py-2 rounded-xl bg-slate-800 text-white font-bold text-sm shadow-lg active:scale-95 transition">Dùng thử miễn phí</button>
      </div>
...
const getNodeClass = (level, type) => {
  ...
  else if (type === 'context') classes += "bg-blue-500 shadow-[0_6px_0_#1d4ed8]";
  else if (type === 'emotion_training') classes += "bg-orange-400 shadow-[0_6px_0_#c2410c]";
