<template>
  <div class="min-h-screen md:pl-64 pt-6 bg-slate-50">
    ...
    <div class="max-w-2xl mx-auto px-6 pb-20">
      <!-- Loading State (Tùy chọn: hiển thị khi chưa tải xong dữ liệu) -->
      <div v-if="isLoading" class="text-center p-10 text-slate-400">
         Đang tải thông tin...
      </div>
      ...
      <div v-else>
        ...
        <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100 flex flex-col md:flex-row items-center gap-8 mb-8">
          <div class="relative">
            <!-- Avatar từ store -->
            <img :src="store.userInfo.avatar || 'https://i.pravatar.cc/300'"
...
const fetchUserProfile = async () => {
  ...
  if (!token) {
    router.push('/login'); // Nếu không có token thì về trang login
    return;
  }
  ...
  try {
    ...
    // Cập nhật dữ liệu vào Store (Pinia)
    // Lưu ý: Trong <script setup>, KHÔNG dùng 'this'
    ...
    // Nếu token lỗi (401/403) -> logout luôn
...
// Gọi hàm fetch khi component được mounted (tải xong)
onMounted(() => {
  fetchUserProfile();
});
